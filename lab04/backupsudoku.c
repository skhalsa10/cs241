...
/*int copyConstraints(int copyToThisGrid[9][9])
{
  int i = 0;
  int j = 0;
  for(i = 0; i<9;i++)
...
int complexSolution()
{
  ...
  int cellWithNineC = -1;
  /*int copyOfConstraintGrid[9][9];

  int copyConstraints(copyOfConstraintGrid);*/
  ...
  if((constraintGrid[i][j] == 0) || !(checkIfGridLegal()))
  {
    /*printf("(localConstraintGrid[i][j] == 0) || !(checkIfGridLegal()) is TRUE");*/
    return 0;
  ...
  for(i = 0; i <3; i++)
  {
    for(j = 0; j <3; j++)
    {
      if(constraintGrid[(3*boxRow + i)][3*boxColumn+j] > 0)
      {
        ...
        {
          case 1:
          if(constraintGrid[(3*boxRow + i)][3*boxColumn+j] & ONE)
          {
            constraintGrid[(3*boxRow + i)][3*boxColumn+j] ^= ONE;
          }
          break;
